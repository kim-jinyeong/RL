...
from app.services.environment import Environment
from app.services.agent import Agent
from app.networks import Network, LSTMNetwork, CNN
...
class ReinforcementLearner:
    ...
    def save_models(self):
        if self.value_network is not None and self.value_network_path is not None:
            ...
            print(f'1번 : {self.value_network_path}')
            self.value_network.save_model(self.value_network_path)
        if self.policy_network is not None and self.policy_network_path is not None:
            self.policy_network.save_model(self.policy_network_path)
            print(f'2번 : {self.policy_network_path}')
    def predict(self):
        # 에이전트 초기화
        self.agent.reset()
